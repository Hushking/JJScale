...
class ApproverRepository{
    async GetProposal() {
      ...
      async Insert(dados){
        return new Promise(async(resolve, reject) => {
          try{
            await poolPromise.query('INSERT INTO APROVADOR (nome, sobrenome, email, telefone, idcliente, cpf_cnpj, status) VALUES ($1, $2, $3, $4, $5)', [dados.nome, dados.sobrenome, dados.email, dados.telefone, dados.idcliente, dados.cpf_cnpj, 1],(err, res) =>{
              if (err == null) {
      ...
      async Edit(dados){
        return new Promise(async(resolve, reject) => {
          try{
            await poolPromise.query('UPDATE APROVADOR SET NOME = $2, SOBRENOME = $3, EMAIL = $4, TELEFONE = $5, IDCLIENTE = $6, CPF_CNPJ = $7 WHERE IDAPROVADOR = $1', [dados.idaprovador, dados.nome, dados.sobrenome, dados.email, dados.telefone, dados.idcliente, dados.cpf_cnpj],(err, res) =>{
              if (err == null) {
      ...
      async Delete(id){
        return new Promise(async(resolve, reject) => {
          try{
            await poolPromise.query('UPDATE APROVADOR SET STATUS = $2 WHERE IDAPROVADOR = $1', [id, 0] ,(err, res) =>{
              if (err == null) {
